...
await Promise.all(
  files.map(async (dataFile) => {
    ...
    outputContent += `    }\n`;
    outputContent += `    \n`;
    outputContent += `    if (!response.ok) throw new Error(\`Failed to fetch ${dataFile}: \${response.status}\`);\n`;
    outputContent += `    \n`;
    outputContent += `    const data = await response.json() as Default;\n`;
    ...
    outputContent += `  } finally {\n`;
    outputContent += `    agent.destroy();\n`;
    outputContent += `  }\n`;
    outputContent += `}\n`;
    outputContent += '\n';
